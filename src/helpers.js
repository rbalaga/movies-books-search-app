...
export const checkGeneres = (selectedGenres, mediaGenres) => {
  const machingGenre = selectedGenres.find((media) =>
    mediaGenres.includes(media.value)
  );
  return machingGenre !== undefined;
};

export const checkYear = (selectedYears, year) => {
  return selectedYears.find((yrObject) => yrObject.value === year);
};
...
export const searchMedia = (searchText, title, year) => {
  return (
    title.toUpperCase().indexOf(searchText.toUpperCase()) >= 0 ||
    year.toUpperCase().indexOf(searchText.toUpperCase()) >= 0
  );
};
